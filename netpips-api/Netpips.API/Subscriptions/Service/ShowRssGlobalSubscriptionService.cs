...
namespace Netpips.API.Subscriptions.Service;
...
{
    private readonly ILogger<ShowRssGlobalSubscriptionService> _logger;
    ...
    {
        _logger.LogInformation("Subscribing to show " + showId);
        ...
        using (var client = new HttpClient(handler))
        {
            ...
            {
                _logger.LogError("SubscribeToShow failed " + (int)response.StatusCode + " " + response.ReasonPhrase);
    ...
    {
        _logger.LogInformation("Unsubscribing to show " + showId);
        using (var handler = new HttpClientHandler() { UseCookies = true, AllowAutoRedirect = true, CookieContainer = context.CookieContainer })
        using (var client = new HttpClient(handler))
        {
            client.DefaultRequestHeaders.Add("User-Agent", OsHelper.UserAgent);
    ...
    {
        _logger.LogInformation("ShowRssService: Authenticate");
        subscriptions = null;
        ...
        using (var client = new HttpClient(handler))
        {
            ...
            var credentialsData = new Dictionary<string, string> { { "username", _settings.Username }, { "password", _settings.Password } };
            ...
            if (!response.IsSuccessStatusCode)
            {
                _logger.LogError("Authenticate failed " + (int)response.StatusCode + " " + response.ReasonPhrase);
                return null;
            }
            var html = response.Content.ReadAsStringAsync().Result;
