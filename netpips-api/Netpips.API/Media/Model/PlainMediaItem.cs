...
public class PlainMediaItem : IEqualityComparer<PlainMediaItem>
{
    ...
    public string Path => HttpUtility.UrlDecode(new Uri(FileSystemInfo.FullName).AbsolutePath.Substring(new Uri(_basePath).AbsolutePath.Length + 1));
    ...
    public string Parent
    {
        get
        {
            ...
            return HttpUtility.UrlDecode(new Uri(System.IO.Path.GetDirectoryName(FileSystemInfo.FullName)).AbsolutePath.Substring(new Uri(_basePath).AbsolutePath.Length + 1));
        }
    }

    [JsonIgnore]
    public MediaItem ToMediaItem => new() { Path = Path, Parent = Parent, Size = Size};
    ...
    [JsonIgnore] private bool IsRootMediaFolder => Path.Split('/').Length < 2;
