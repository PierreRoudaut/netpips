...
public class P2PDownloadMethod : IDownloadMethod
{
    ...
    private readonly IAria2CService _aria2CService;
    private readonly ILogger<P2PDownloadMethod> _logger;
    ...
    public P2PDownloadMethod(ILogger<P2PDownloadMethod> logger, IOptions<NetpipsSettings> settings, IAria2CService aria2CService, ITorrentDaemonService torrentDaemonService)
    {
        ...
        _logger = logger;
        _aria2CService = aria2CService;
        _settings = settings.Value;
    }

    public bool Cancel(DownloadItem item) => _torrentDaemonService.StopTorrent(item.Hash);


    public long GetDownloadedSize(DownloadItem item) => _torrentDaemonService.GetDownloadedSize(item.Hash);
    ...
    private void DownloadMagnetLink(string magnetLink, string torrentFolder)
    {
        ...
        stopWatch.Start();
        var exitCode = _aria2CService.DownloadTorrentFile(magnetLink, torrentFolder, TimeSpan.FromSeconds(120));
        stopWatch.Stop();
        _logger.LogInformation("aria2c duration: " + stopWatch.Elapsed.Seconds + "," + stopWatch.Elapsed.Milliseconds + "s");
        if (exitCode == -1)
        {
    ...
    private void DownloadTorrentFile(string torrentUrl, string torrentFolder)
    {
        using (var client = new WebClient())
        {
            try
            {
                _logger.LogInformation("Downloading torrent url: " + torrentUrl);
                client.DownloadFile(new Uri(torrentUrl), Path.Combine(torrentFolder, Path.GetRandomFileName() + ".torrent"));
                _logger.LogInformation("SUCCESS");
            }
            catch (System.Exception)
            {
                _logger.LogInformation("FAILURE torrent url could not be downloaded");
    ...
    private Torrent ParseTorrent(string torrentPath)
    {
        ...
        {
            _logger.LogWarning(TorrentFileCorrupted + " ex: " + ex.Message);
    ...
    public void Start(DownloadItem? item)
    {
        ...
        try
        {
            ...
            if (!_torrentDaemonService.AddTorrent(torrentPath, itemFolder))
    ...
    {
        return _torrentDaemonService.RemoveTorrent(item.Hash);
    }

    public static List<Regex> SupportedUrls = new()
    {
        new Regex(@"magnet:?.*")
    };
